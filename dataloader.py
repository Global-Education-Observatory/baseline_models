...
class Dataloader():
    ...
    def __init__(self, image_names, dta, records_dir, batch_size = 16, split = .75):
        ...
        val_indices = list(set([i for i in range(0, len(image_names))]).difference(train_indices))        
        
        self.train_images = list(map(image_names.__getitem__, train_indices))
        self.val_images = list(map(image_names.__getitem__, val_indices))
        
        # saving the validation images for the final forward pass
        with open(f"{records_dir}/valimages.txt", "a") as f:
            [f.write(_ + "\n") for _ in self.val_images]
        
        # split training and testing into batches
...
#         get normalization statstics for the imagery
    ...
    def shuffle_imagery(self):
        ...
        """Shuffle imagery at the start of each batch"""
    ...
    def get_range(self, phase):
        ...
        """Get the number of batches in training/validation"""
    ...
    def get_num(self, phase):
        ...
        """Get number of images in training/validation"""
...
#         print(self.image_names[idx].split("/"))
        
#         dhsid = str(self.image_names[idx].split("/")[10])
        x = torch.cat([self.load_image(self.image_names[idx])])
        y = torch.tensor([self.dta[self.image_names[idx]]])
                            
        return (x, y)
    
    
    
class Transfer_Valoader():
    
    def __init__(self, image_names, dta, records_dir, iso, dta_iso, pooling = False):
